...
def store_access_token(db: Session, access_token: str):
    try:
        # Check if the data already exists
        # existing_data = db.query(UserInstagramData).filter_by(user_id=user_id).first()
...
def store_instagram_data(db: Session,facebook_page_id: str,  instagram_user_id: str):
    try:
        # Check if the data already exists
        existing_data = db.query(User).filter_by(id=1).first()
